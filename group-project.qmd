...
Fecaldata <- read.csv(file = "Fecaldata.csv")

Fecaldata %>%
  filter(
    !is.na(sex), 
    !is.na(disease_category), 
...
This figure was created to display a distribution of the ages of subjects used in this data set. Roughly 60 subjects were excluded from this plot as they failed to report an age. The data follows a distinct bell shaped curve with a majority of samples falling between 8 and 11 years old. The plot helps demonstrate that a wide variety of ages were used in the samples collected which furthers the reliability of the subsequent database.

```{r}
Fecaldata %>%
  filter(!is.na(age_of_collection)) %>%
  mutate(age_years = age_of_collection / 365) %>%
